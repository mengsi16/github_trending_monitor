...
项目内置 QA Agent，支持针对历史热榜数据做自然语言问答，而不只是生成 summarize。当前问答链路采用**多源信息获取策略**：

1. **RAG 检索**：基于 ChromaDB 对 `latest` 记录进行语义搜索
2. **Playwright 缓存**：Playwright 浏览器访问的页面快照自动保存在 `.playwright-mcp/` 目录，Agent 可用 `grep`/`read_file` 直接读取缓存，避免重复浏览
3. **Playwright 网页研究**：RAG 和缓存都没有数据时，Agent 自动用 Playwright 访问 GitHub 等网页获取实时信息
4. **本地文件访问**：Agent 可读取项目目录下的文件、执行 Shell 命令，直接访问爬取的原始数据
5. **自动补数**：当知识库为空或不足时，QA 可以请求触发爬虫更新

此外，问答链路还包含：

- **轻量 Query 改写**：`rag_search` 会自动提取 repo 名、清洗停用词、构造多个检索变体并合并去重
- **会话持久化**：QA 会话保存在 SQLite 中，可继续历史对话
- **迭代干预**：Agent 循环接近上限时自动注入收尾指令，防止工具调用死循环
- **优雅降级**：超过迭代上限时提取已有文本返回，而非抛出异常

这意味着系统已经具备"先查 RAG → 再查缓存 → 再浏览网页 → 最后回答"的完整能力链，而不仅仅是 summarize-only 的管道。

### 可选：接入 brain-base 知识底座（解耦集成）

**brain-base** 是独立的 Agentic RAG 知识库项目，提供 Milvus 向量检索、Agentic 回答、doc2query 合成、自进化固化等能力。github-trending-monitor 与 brain-base **代码完全解耦**：

- 本项目**不内置 brain-base 代码**，也不复制它的 skill 文件
- 仅通过环境变量 `BRAIN_BASE_PATH` 指向 brain-base 仓库根目录
- QA 启动时由 `SkillLoader` 动态加载 brain-base 的 skill 指令，`brain_base` 工具通过 subprocess 调用 `brain-base-cli.py`
- 未设置 `BRAIN_BASE_PATH`、brain-base 服务异常或查不到结果时，QA 自动降级到 ChromaDB + 网页浏览链路
- brain-base 升级（例如新增 CLI 命令或 skill 指令）后，本项目**不需要任何代码同步**，重启即可获得最新能力

集成分两段：

1. **skill 指令集成**：`skills/brain-base/SKILL.md` 是薄指针文件，运行时由 `SkillLoader` 通过 `external-skill: true + source-path` 字段从 `BRAIN_BASE_PATH` 精确读取 brain-base 上游 skill 正文
2. **工具函数集成**：`src/tools/brain_base.py` 只封装高频命令（`search` / `ask` / `exists` / `ingest-url` / `enrich-chunks`）；低频命令（`feedback` / `resume` / `history` / `remove-doc` 等）Agent 通过 skill 指令直接调用 `brain-base-cli.py` 即可
...
- **通道级会话隔离**：每个 `channel:peer_id` 组合（如 `feishu:oc_xxx`、`cli:default`、`email:user@xxx`）拥有独立的会话上下文，不同通道的消息历史互不干扰，避免跨通道迭代深度消耗和上下文污染
- **线程安全**：QAAgent 内部通过 `threading.Lock` 保护并发访问，飞书轮询线程与 CLI 主线程可安全并行
- **消息历史校验**：Agent 循环在每次 API 调用前自动校验 `tool_use`/`tool_result` 配对关系，修复因上下文压缩导致的孤立引用，避免 `tool_use_id not found` 错误
...
Playwright 浏览器访问的页面快照会自动保存在 `.playwright-mcp/` 目录下，QA Agent 会优先检查缓存再发起新的浏览器访问，避免重复请求。

`mcp_servers.json` 中的服务器在系统启动时自动连接，工具动态注册到 QA Agent 的工具调用链中。若不需要任何 MCP 工具，跳过此步即可，系统正常运行不受影响。

### 4. 工作区文件工具

QA Agent 内置了工作区文件与 Shell 工具（`src/tools/workspace.py`），无需额外配置：

| 工具 | 用途 | 安全限制 |
|------|------|----------|
| `read_file` | 读取项目目录下的文件 | 限定项目根目录 |
| `list_dir` | 列出目录内容 | 限定项目根目录 |
| `grep` | 正则搜索文件内容 | 限定项目根目录，跳过 >5MB 文件 |
| `bash` | 执行 Shell 命令 | Windows 使用 PowerShell，Linux/macOS 使用 Bash；危险命令黑名单拦截 |
| `edit_file` | 编辑文件（查找替换） | 只允许修改 `workspace/` 和 `.playwright-mcp/` 下的文件 |

### 5. 可选：接入 brain-base 知识底座

如果你希望 QA 优先使用 brain-base 的 Agentic RAG 能力，只需：

1. 在另一处 clone brain-base 仓库（例如 `~/repos/brain-base`），并按 brain-base 自己的 README 启动 Milvus
2. 在当前项目中设置环境变量指向 brain-base 根目录：

   ```bash
   # Linux / macOS
   export BRAIN_BASE_PATH="/absolute/path/to/brain-base"
   export BRAIN_BASE_CLAUDE_BIN="claude"   # 可选：自定义 claude 可执行文件
   ```
...
- 不设置 `BRAIN_BASE_PATH` → `brain_base` 工具直接返回错误、`load_skill("brain-base")` 返回 `BRAIN_BASE_PATH is not set`，QA Agent 会自动走 ChromaDB + 网页浏览链路，不会阻断主流程
- 本项目**只扫描自身 `skills/` 目录**，不会扫描外部路径；brain-base 的完整 skill 正文仅在显式 `load_skill` 时按 `source-path` 精确读取，避免任意文件读取风险
- brain-base 升级时不需要手动同步本项目：只要 skill 指针字段（`source-path`）不变，重启就能拿到最新 skill 正文与 CLI 命令说明
...
| `QAAgent` | 多源信息获取（RAG + Playwright + 文件工具），回答问题，维护通道级隔离会话历史 |
...
| `WorkspaceTools` | 提供文件读取、目录浏览、文本搜索、Shell 执行、文件编辑能力 |
| `MemoryWatchdog` | 周期性监控进程内存，超阈值自动快照或重启，作为 Tool 暴露 `memory_snapshot` |
| `LaneQueue` | Lane 并发控制，用户输入优先于后台任务（Heartbeat 等） |
| `SkillLoader` | 扫描本项目 `skills/` 目录，支持 `external-skill: true` 薄指针按需从 `BRAIN_BASE_PATH` 加载上游 skill 正文；暴露 `list_skills` / `load_skill` 工具 |
| `BrainBaseTool` | 封装 `brain-base-cli.py` 高频命令（`search` / `ask` / `exists` / `ingest-url` / `enrich-chunks`），未配置 `BRAIN_BASE_PATH` 时优雅失败并降级 |

在实现上，系统沿用了 ReAct Agent 循环、工具调用、上下文压缩和 Circuit Breaker 机制，以便在长对话、外部依赖失败和多渠道接入的情况下保持稳定。同时，Agent 循环具备迭代干预与优雅降级机制，飞书 WebSocket 长连接支持断线自动重连。多渠道并发场景下，QAAgent 通过通道级会话隔离和线程安全锁确保不同通道的用户互不干扰；消息历史校验机制在每次 API 调用前自动修复上下文压缩产生的 `tool_use`/`tool_result` 不匹配问题。
...
- **brain-base**（Agentic RAG 知识库，独立仓库，`BRAIN_BASE_PATH` 环境变量启用）：无需加入 `requirements.txt`；只需本机已 clone 其仓库并按其 README 启动 Milvus 即可
