...
| `QAAgent` | 多源信息获取（RAG + Playwright + 文件工具），回答问题，维护会话历史 |
...
在实现上，系统沿用了 ReAct Agent 循环、工具调用、上下文压缩和 Circuit Breaker 机制，以便在长对话、外部依赖失败和多渠道接入的情况下保持稳定。同时，Agent 循环具备迭代干预与优雅降级机制，飞书 WebSocket 长连接支持断线自动重连。
