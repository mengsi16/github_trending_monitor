...
Gateway 是消息路由的核心枢纽，接收来自各个 Channel 的 InboundMessage，根据消息来源、目标账户和渠道类型决定将消息路由到哪个 Agent 处理。Binding 表维护 channel、account、peer 到 Agent 的映射关系，支持复杂的路由规则，比如特定飞书群的消息路由到 SummarizerAgent，特定邮件地址的消息路由到 QAAgent。
